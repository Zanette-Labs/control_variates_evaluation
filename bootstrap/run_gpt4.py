from utils import DualLogger, ResultGrouper
...
def get_args():
    ...
    parser.add_argument('--rm_dict_file_path', type=str, required=True)
    ...
    # parser.add_argument('--threshold', type=int, default=0, help="Threshold for the number of samples in model pairs")
    parser.add_argument('--bt', action="store_false", help="Use BT modelling")
    # parser.add_argument('-e', '--eval_type', type=str, required=True, help="Type of evaluation: rm, human, cv")
...
def evaluate_pair(model_pair, eval_type, evaluator, num_samples, gt_winrate):
    # rm_scores, gt_scores = rm_dict[model_pair], gt_dict[model_pair]
    # if num_samples < args.num_corr_samples:
    #     return model_pair, None
    print(f"Evaluating {model_pair}")
    evaluator.clear_result()
    for num_gt_samples in range(10, 1001, 1):
...
if __name__ == "__main__":
    ...
    # Load data
    data = utils.load_pickle_data(args.rm_dict_file_path, ambiguous=False)
    rm_scores = data["api_score"]
    gt_scores = data["gt_score"]
    model_a_names = data["model_a"]
    model_b_names = data["model_b"]
