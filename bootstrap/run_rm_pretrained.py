from utils import load_rm_rewards_v2, get_rm_score, DualLogger, ResultGrouper
...
if __name__ == "__main__":
    ...
    assert len(model_a_names) == 1
    assert len(model_b_names) == 1
    assert len(rm_scores) == 1
    assert len(gt_scores) == 1
    # process data
    grouper = utils.ResultGrouper(
        model_a_arr = model_a_names[0],
        model_b_arr = model_b_names[0],
        rm_scores = rm_scores[0],
        gt_scores = gt_scores[0],
